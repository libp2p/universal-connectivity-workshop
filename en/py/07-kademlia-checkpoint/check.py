#!/usr/bin/env python3
"""
Check script for Lesson 7: Kademlia Checkpoint
Validates that the student's solution can subscribe bootstrap kademlia and get closest peers
"""
...
def validate_multiaddr(addr_str):
    ...
    if not ("/tcp" in addr_str or "/udp" in addr_str or "/quic-v1" in addr_str):
        return False, f"Missing TCP/UDP/QUIC transport in multiaddr: {addr_str}"
     
    return True, f"Valid multiaddr: {addr_str}"


def check_output():
    """Check the output log for expected kademlia checkpoint functionality"""
    ...
    try:
        ...
        print("i Checking kademlia checkpoint functionality...")
        
        if not output.strip():
            print("x checker.log is empty - application may have failed to start")
            return False
        
        print(f"i Checker output: {repr(output[:200])}")  # Debug: show first 200 chars
        ...
        addr = connected_matches.group(2)
        valid, addr_message = validate_multiaddr(addr)
        if not valid:
            print(f"x {addr_message}")
            return False

        print(f"v Connection established with {peerid_message} at {addr_message}")
...
def main():
    """Main check function"""
    print("i Checking Lesson 7: Kademlia Checkpoint 🏆")
    print("i " + "=" * 50)
    ...
    try:
        ...
        print("i " + "=" * 50)
        print("y Kademlia checkpoint completed successfully! 🎉")
        print("i You have successfully:")
        print("i • Configured Kademlia for bootstrapping and peer discovery")
        print("i • Reached your fourth checkpoint!")
        print("Ready for Lesson 8: Universal Connectivity!")
